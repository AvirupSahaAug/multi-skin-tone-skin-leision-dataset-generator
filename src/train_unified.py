...
def train(args):
    ...
    if args.pretrained_classifier:
        ...
        C.load_state_dict(torch.load(args.pretrained_classifier, map_location=device))
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        for i, (real_imgs, lesion_labels) in pbar:
            ...
            for t, fake_img in enumerate(fake_imgs_list):
                ...
                # 2. Tone Loss (Push generated image to be classified as Tone 't' by C)
                # target tone label is 't' for every image in this batch for this head
            ...
            g_loss = g_adv_loss + (args.lambda_tone * g_tone_loss)
            g_loss.backward()
            optimizer_G.step()
