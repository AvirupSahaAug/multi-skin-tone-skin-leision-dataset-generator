...
class MultiHeadGenerator(nn.Module):
    def __init__(self, z_dim=100, num_classes=7, img_size=128, channels=3, conv_dim=64):
        ...
        # 3 Independent Heads
        self.head1 = self._make_head(conv_dim, channels)
        self.head2 = self._make_head(conv_dim, channels)
        self.head3 = self._make_head(conv_dim, channels)
    ...
    def forward(self, z, labels):
        ...
        x1 = self.head1(features)
        x2 = self.head2(features)
        x3 = self.head3(features)
