...
def train(args):
    ...
    # Use the STRONG classifier (ResNet18)
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        for i, (real_imgs, lesion_labels) in pbar:
            ...
            # If generator adversarial loss spikes, scale down the auxiliary losses 
            # so the generator can focus on fooling the discriminator.
            adv_loss_val = g_adv_loss.item()
            scale_factor = 1.0
            if adv_loss_val > 3.2:  
                # scales down proportionately, but never goes below 0.5 (half)
                scale_factor = max(0.5, 3.2 / adv_loss_val)

            curr_lambda_tone = args.lambda_tone * scale_factor
            curr_lambda_cons = args.lambda_consistency / scale_factor
            # curr_lambda_cons = args.lambda_consistency
        ...
        if epoch % args.checkpoint_interval == 0:
            torch.save(G.state_dict(), f"{args.checkpoint_dir}/G_strong_unified_{epoch}.pth")
            torch.save(D.state_dict(), f"{args.checkpoint_dir}/D_strong_unified_{epoch}.pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_classifier", type=str, default="checkpoints/unified/C_strong_best.pth")
    parser.add_argument("--csv_path", type=str, default="data/processed/ham10000_train.csv")
    parser.add_argument("--sample_dir", type=str, default="samples/strong_unified")
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints/strong_unified")
